...
describe('Sales Record app', function () {
  ...
  it('opens a window displaying the nav and the record sale page', function () {
    return app.client.getWindowCount().should.eventually.equal(1)
      ...
      .browserWindow.getTitle().should.eventually.equal('Sales Record')
      .waitForVisible('.js-nav').should.eventually.be.true
      .isExisting('button.is-selected[data-section="record"]').should.eventually.be.true
  })
  ...
  describe('when clicking on a section from the nav bar', function () {
    it('it shows the selected section in the main area', function () {
      return app.client/* .dismissAboutPage() */
        ...
        .isExisting('button.is-selected[data-section="history"]').should.eventually.be.true
        .isVisible('#record-section').should.eventually.be.false
        .selectSection('record')
        .isVisible('#history-section').should.eventually.be.false
        .isExisting('button.is-selected[data-section="history"]').should.eventually.be.false
        .isExisting('button.is-selected[data-section="record"]').should.eventually.be.true
    })
  })
  /*
